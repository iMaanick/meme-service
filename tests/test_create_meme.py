...
def test_create_meme_success(client: TestClient, mock_database_gateway: AsyncMock,
...
def test_create_meme_missing_description(client: TestClient, monkeypatch: MonkeyPatch) -> None:
    src_dir = os.path.normpath(os.path.join(__file__, os.path.pardir))
    with open(os.path.join(src_dir, "test_files/test_file.jpg"), "rb") as test_file:
        response = client.post(
            "/memes/",
...
def test_create_meme_invalid_file_type(client: TestClient) -> None:
    src_dir = os.path.normpath(os.path.join(__file__, os.path.pardir))
